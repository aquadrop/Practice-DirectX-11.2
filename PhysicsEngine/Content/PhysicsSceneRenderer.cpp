...
PhysicsSceneRenderer::PhysicsSceneRenderer(const std::shared_ptr<DX::DeviceResources>& deviceResources, Geometry^ model) :
...
void PhysicsSceneRenderer::CreateWindowSizeDependentResources()
{
...
void PhysicsSceneRenderer::Update(DX::StepTimer const& timer)
{
	if (!m_tracking)
	{
...
void PhysicsSceneRenderer::Rotate(float spin_radians, float rotate_radians)
{
	// Prepare to pass the updated model matrix to the shader
	//XMStoreFloat4x4(&m_constantBufferData.model, XMMatrixTranspose(XMMatrixRotationY(radians)));
	this->model->Rotate(m_constantBufferData, spin_radians, rotate_radians);
}

void PhysicsSceneRenderer::StartTracking()
{
	m_tracking = true;
}

// When tracking, the 3D cube can be rotated around its Y axis by tracking pointer position relative to the output screen width.
void PhysicsSceneRenderer::TrackingUpdate(float positionX, float positionY)
{
	if (m_tracking)
	{
...
void PhysicsSceneRenderer::StopTracking()
{
	m_tracking = false;
}

// Renders one frame using the vertex and pixel shaders.
void PhysicsSceneRenderer::Render()
{
...
void PhysicsSceneRenderer::CreateDeviceDependentResources()
{
	...
	auto createCubeTask = (createPSTask && createVSTask).then([this]() {
		...
		vertexBufferData.pSysMem = this->model->getVertexPositionColor();
		vertexBufferData.SysMemPitch = 0;
		vertexBufferData.SysMemSlicePitch = 0;
		CD3D11_BUFFER_DESC vertexBufferDesc(sizeof(this->model->getVertexPositionColor()[0])*this->model->n_color_vertex, D3D11_BIND_VERTEX_BUFFER);
		DX::ThrowIfFailed(
			m_deviceResources->GetD3DDevice()->CreateBuffer(
			&vertexBufferDesc,
			&vertexBufferData,
		...
		indexBufferData.pSysMem = this->model->getMeshColorIndex();
		indexBufferData.SysMemPitch = 0;
		indexBufferData.SysMemSlicePitch = 0;
		CD3D11_BUFFER_DESC indexBufferDesc(sizeof(this->model->getMeshColorIndex()[0])*m_indexCount, D3D11_BIND_INDEX_BUFFER);
		DX::ThrowIfFailed(
			m_deviceResources->GetD3DDevice()->CreateBuffer(
			&indexBufferDesc,
			&indexBufferData,
...
void PhysicsSceneRenderer::ReleaseDeviceDependentResources()
{
	m_loadingComplete = false;
